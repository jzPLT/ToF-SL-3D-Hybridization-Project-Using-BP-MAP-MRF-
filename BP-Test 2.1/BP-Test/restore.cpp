...
image<uchar> *restore_ms(image<uchar> *img1, image<uchar> *img2) {
...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < img1->width() * img1->height(); i++) {
		std::cout << (int)img1->data[i];
		std::cout << " ";
	}
	*/

	/*//*** Geometric Depth Calibration, probably goin to be in BP-test for 2 images.
	float** depdif = new float*[img1.rows];
	for (int i = 0; i < img1.rows; i++) {
		depdif[i] = new float[img1.cols];
	}

	for (int i = 0; i < img1.rows; i++) {
		for (int j = 0; j < img1.cols; j++) {
			//depdif[i][j] = img1.data[i][j] - img2[i][j];
		}
	}
	//*******/

	// restore image from both inputs
	out = restore_ms(img1, img2);
